...
int rotating_state_machine(uint8_t destination);


uint8_t JUNCTION_STATE = IDLE;
uint8_t ROTATING_STATE = ENTRY;
...
void loop(){
    ...
    if(wifiprev!=wifi_current_state||soceketprev!=current_socket_state||junctionprev!=JUNCTION_STATE){
        Serial.printf("wifi:%d|socket:%d|client:%d \n",wifi_current_state,current_socket_state,JUNCTION_STATE);
...
int junctionStateMachine(){
    ...
    static uint8_t destination = STRAIGHT;
